    ...
    <v-container fluid>
      ...
      <v-row v-if="info">
        <v-col :sm="12" :lg="6" class="order-10 order-lg-0">
          ...
          >
            <slide :index="i" :key="i" v-for="(track, i) in tracks">
              <figure>
                <img :src="track.cover" />
                <figcaption>
                  <v-btn text color="white">{{ track.artist }}</v-btn>
                </figcaption>
              </figure>
            </slide>
          </carousel-3d>
          ...
          >
            ...
            <v-card-title>Music Info</v-card-title>
            <v-card-text>
              ...
              <v-row>
                <v-card-title
                  class="pa-1 ma-0"
                  :class="{ 'black--text': switch1 }"
                  :style="{
                    color: switch1 ? '#f1f1f1' : color.color5,
                  }"
                  >Name :
                </v-card-title>
                <v-card-title class="text-subtitle-3 pa-1 ma-0">
                  {{ info.title }}
                </v-card-title>
              </v-row>
              <v-row>
                ...
                <v-card-title class="text-subtitle-2 pa-1 ma-0">
                  {{ info.Released }}
                </v-card-title>
              </v-row>
              <v-row>
              ...
              <v-row>
                <v-sheet style="width: 100%">
                  ...
                  >
                    <v-card-text>
                      <h3 class="font-weight-black font-italic">
                        Be The First One To Comment
                      </h3>
                    </v-card-text>
                  </v-card>

                  <v-row :key="p" v-for="(comment, p) in comments">
                    <v-card class="ma-0 pa-0" style="width: 100%">
                      <v-list-item>
                        <v-list-item-content>
                          <v-list-item-title
                            class="
                          ...
                          >
                            ...
                            <v-card style="border-radius: 25px">
                              <v-btn
                                ...
                                "
                                @click="
                                  closeEditBtn(comment.id, currentTrackIndex, p)
                                "
                              >
                                x
                              </v-btn>
                              <v-form id="editForm" v-model="valid">
                                ...
                                <button
                                  ...
                                  @click.prevent="
                                    editComment(currentTrackIndex, p)
                                  "
                                >
                          ...
                          >
                            ...
                            <template v-slot:action="{ attrs }">
                              <v-btn
                                ...
                                text
                                v-bind="attrs"
                                @click="deleteComment(currentTrackIndex, com)"
                              >
                                Yes
                              </v-btn>
                              <v-btn
                  ...
                  <v-sheet>
                    <v-dialog
                      v-model="dialog[currentTrackIndex]"
                      width="600px"
                      heigh="fit-contain"
                      :retain-focus="false"
                      dark
                      ...
                      <v-card style="border-radius: 25px">
                        <v-btn
                          ...
                          style="
                            ...
                            right: 0px;
                          "
                          @click="closeDialog(currentTrackIndex)"
                        >
                          x
                        </v-btn>
                        <v-form id="form" ref="form1" v-model="valid">
                          <v-card-text class="mt-5">
                            <div class="reply">
                              <v-text-field
                                tag="input"
                          ...
                          <button
                            ...
                            type="submit"
                            class="reply--button mb-5"
                            @click.prevent="submitComment(currentTrackIndex)"
                          >
                            <i class="fa fa-paper-plane"></i> Send &nbsp;
                          </button>
                        </v-form>
        ...
        <v-col :sm="12" :lg="6">
          <div class="wrapper" id="app">
            <div
              class="player"
              :style="{ backgroundColor: switch1 ? '' : color.color1 }"
            >
              <div class="player__top">
                ...
                <div class="player-controls">
                  <div
                    ...
                    :class="{ active: currentTrack.likes.bol }"
                    @click="favorite"
                  >
                    <svg class="icon">
              ...
              <div class="progress" ref="progress">
                <div class="progress__top">
                  <div class="album-info" v-if="currentTrack">
                    <div
                      class="album-info__name"
                      :style="{ color: switch1 ? '' : color.color5 }"
                    >
                      {{ currentTrack.artist }}
                    </div>
                    <div
                      class="album-info__track"
                      :style="{ color: switch1 ? '' : color.color4 }"
                    >
                      {{ currentTrack.title }}
                    </div>
              ...
              <v-card class="mx-auto" max-width="400">
                <v-list
                  :dark="!switch1"
                  :style="{ backgroundColor: switch1 ? '' : color.color2 }"
                >
                  <v-list-group
                    sub-group
                    prepend-icon="mdi-playlist-music"
                    :color="switch1 ? color.color2 : 'green'"
                  >
                    <template v-slot:activator>
                      <v-list-item-content
                        :style="{
                          color: switch1 ? color.color1 : color.color5,
                        }"
                      >
                    ...
                    <v-list-item-group
                      v-model="model"
                      :color="switch1 ? color.color3 : 'green'"
                    >
                      <v-list-item
                      ...
                      >
                        <v-list-item-icon>
                          <v-icon
                            v-if="!isTimerPlaying && currentTrackIndex == i"
                            @click="play()"
                            >mdi-play</v-icon
                          >
                          <v-icon
        ...
        <v-sheet v-if="volumeBtn" class="bg-light volumeWarper">
          <v-expand-transition>
            <v-slider
          ...
          <v-icon large light @click="volume = !volume">mdi-volume-high</v-icon>
        </v-sheet>
      </v-row>
      <Message />
    </v-container>
...
export default {
  ...
  methods: {
    play() {
      if (this.audio.paused) {
        ...
        this.volumeBtn = false;
      }
    },
    setVolume() {
      this.audio.volume = this.media / 100;
    ...
    favorite() {
      this.tracks[this.currentTrackIndex].likes.bol =
        !this.tracks[this.currentTrackIndex].likes.bol;
      var i = this.currentTrackIndex;
      if (this.tracks[i].likes.bol == true) {
      ...
      localStorage.setItem(
        `MusicLikesOf${i}`,
        JSON.stringify(this.tracks[i].likes)
      );
    },
    share(i) {
    ...
    getComets() {
      ...
      axios.get(base_url).then((responseComment) => {
        ...
        for (let n = 0; n <= this.trackslength; n++) {
          if (window.localStorage.getItem(`MusicCommentsOf${n}`) !== null) {
            ...
            for (let i = 0; i < Comments.length; i++) {
              if (
                JSON.parse(localStorage.getItem(`MusicCommentsOf${n}`))[x] &&
                Comments[i].id ==
                  JSON.parse(localStorage.getItem(`MusicCommentsOf${n}`))[x].id
              ) {
                // console.log(JSON.parse((localStorage.getItem(`MusicCommentsOf${n}`)))[x].id + " "+ i );
                Comments[i].author =
                  // true;
                  JSON.parse(localStorage.getItem(`MusicCommentsOf${n}`))[
                    x
                  ].author;
                x++;
              }
      ...
      this.tracks[this.currentTrackIndex].comments = this.comments;
    },
    submitComment(n) {
      this.comments.push({
        // id: newComment.id,
      ...
      axios
        ...
        .then((response) => {
          ...
          var authorCommnets = this.comments.filter((e) => {
    ...
    editComment(n, p) {
      this.comments[p] = {
        id: this.comments[p].id,
        name: this.Editname,
        message: this.Editmessage,
        Number: n,
        color: this.comments[p].color,
        author: true,
      };
      //api
      const axios = require("axios");
      ...
      this.commentDialog[this.comments[p].id] = false;
    },
    deleteComment(n, p) {
      // api
      console.log(n);
  ...
  computed: {
    comments() {
      var data = this.DataComment.filter((e) => {
        return e.Number == this.commentsIndex;
      });
      return data;
    },
    color() {
...
#inspire {
  min-height: 100%;
  background: url("../assets/img/m2.jpg") rgba(0, 0, 0, 0.4);
